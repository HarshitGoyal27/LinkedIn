...
let scrapped_data=[];
//sap //legacy //cloud
export const runPuppeteer =async(data) => {
  try{
    ...
    await page.type('input[placeholder="Search"]',`${data}`);
...
async function scrapEachPost(arr_li,page,browser){
  const promises = arr_li.map(async (postElement) => {
    ...
    obj.profile_link={};
    obj.profile_link.url=await postElement.$eval('div[class="full-height"]>div>div>div>div>div>a',element=>element.getAttribute('href'));
    // const profilePage = await browser.newPage();
    // await profilePage.goto(`${obj.profile_link.url}`,{ timeout: 60000 });
    // // extractProfileInfo(profilePage);
    // try{
    //   await profilePage.waitForSelector('.mt2.relative>div',{timeout:5000});
    //   let ele=await profilePage.$('.mt2.relative>div');
    //   let second_div=await ele.$$('div>div');
    //   console.log('length is',second_div.length);
    //   obj.profile_link.description=await second_div[1].innerText();
    // }catch(err){
    //   obj.profile_link.description='This is a page';
    //   console.log(err);
    // }
